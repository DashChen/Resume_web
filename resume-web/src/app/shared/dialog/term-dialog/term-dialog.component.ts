...
export class TermDialogComponent {
  ...
  termHtmlContent: SafeHtml = '';

  serviceTerm: string = `<div class="iso"></div>`;
  privacyTerm: string = `<div class="post-content">
  <p>				歡迎來到「傑報人力資源服務集團官方網站」（以下簡稱本網站），為了讓您能夠安心的使用本網站的各項服務與資訊，特此向您說明本網站的隱私權保護政策，以保障您的權益，請您詳閱下列內容：</p>
...
<p>本網站隱私權保護政策將因應需求隨時進行修正，修正後的條款將刊登於網站上。		</p>
  ...
  handleReturn() {
    if (this.showDetail) {
      ...
      this.dialogRef.updateSize('614px', '320px');
    } else {
      this.closeDialog();
    }
  }
  ...
  showTerm(type: string) {
    this.isService = type === 'service';
    this.isPrivacy = type !== 'service';
    this.title = type === 'service' ? '服務政策' : '隱私權政策';
    this.showDetail = true;
    this.termHtmlContent = this.transformSafeHtml(type === 'service' ? this.serviceTerm : this.privacyTerm);
    this.dialogRef.updateSize('614px', '735px');
  }

  transformSafeHtml(value: string) {
